...
class MainServers extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, server_type, settings', 'required'),
			array('name', 'length', 'max' => 50),
			array('server_type', 'length', 'max' => 10),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, server_type, settings', 'safe', 'on' => 'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'groups' => array(self::HAS_MANY, 'Groups', 'server_id'),
		);
	}
